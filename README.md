...
- **参加しているグループカード内のモーダルから作成/参加**:
  「＋作成」「＋参加」ボタンでフォームを開く構成に変更しました。
...
- **シフトを提出する**: イベントを選び、同イベント内の所属グループを複数選択して提出します。
  選択中グループのメンバー一覧が表示されます。ブースは候補付き入力で選択します。
...
- スマホ表示時に地図が潰れないよう、高さとレイアウトをレスポンシブに調整しています

## イベント機能について

- `events.html` のイベント作成は、「登録先イベント」カード内の「＋ イベント追加」ボタンからモーダルで行います
- イベントは単日ではなく、開始日・終了日の複数日定義に対応しました
- ブース/企画一覧は `event-details.html` に分離し、イベント詳細ページで確認する構成にしました
- 「企画の開始/終了時刻」入力は廃止しました
