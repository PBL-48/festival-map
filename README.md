...
- **シフトを提出する**: 所属グループを選ぶと、そのグループ(サークル)のメンバー一覧が
  その場に表示されます。イベントはプルダウン、ブースは候補付き入力で選択します。
  イベントやブースの新規登録は `events.html` にまとめました。
...
`map.html`を追加しました。OpenStreetMap(無料・APIキー不要)とLeaflet.jsを使い、
ズーム・スクロール可能な地図の上にブースをピンで表示します。

- 左側の地図: ブースにカーソルを合わせると、その場所の企画一覧とシフト情報がツールチップで表示されます
- 右側: 「表示するグループ」チェックボックス(デフォルト全選択)と「現在シフト中」の一覧
- ブース追加や企画登録は `events.html` で行い、`map.html` は閲覧専用にしました
- 所属グループが複数のイベントにまたがる場合のみ、上部にイベント切り替えのプルダウンが表示されます
- 「グループ管理」「シフト提出」は右上のリンクから`dashboard.html`/`shifts.html`に移動する形にしています

**設計上の決定事項**: グループは必ず1つのイベントに属する仕様にしました(1グループ=1イベント固定)。
複数のイベントに出る場合は、イベントごとに別のグループを作成してください。

このマップ機能を有効にするには、Supabase側で追加のマイグレーション
(チャット本文に記載したSQL: `groups.event_id`必須化、`booths.lat/lng`追加、
`programs`テーブル新設、`create_group`関数の更新)を実行し、
`dashboard.html`のグループ作成フォームでイベントも選択できるようにした
更新版ファイルに差し替えてください。

## 次のステップ

既存の`index.html`(CSV読み込み版)をこの`map.html`に置き換えて
新しいトップページにするか、しばらく並行運用するか、方針を決めていきましょう。